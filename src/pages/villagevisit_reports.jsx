...
import { getvillagevisitAPI , getPDFAPI } from "../utils/apis";
...
const VillageVisitReportsPage = () => {
    ...
    useEffect(() => {
        console.log("Effect: Filtering tickets");
    ...
    const renderTickets = () => {
        ...
        return filteredTickets.slice(startIndex, endIndex).map(ticket => (
            <tr key={ticket._id}>
                ...
                <td>{ticket.purposeVisit}</td>
                <td>{ticket.gpProgram}</td>
                <td>{ticket.proDesc}</td>
                <td>{ticket.proInchagre}</td>
                <td>{ticket.proInchagrePhone}</td>
            </tr>
        ));
    };

    useEffect(() => {
        villagevisitreports(page);
    }, [page]);

    const villagevisitreports = async (page) => {
        try {
            document.body.classList.add("loading-indicator");
            const offset = (page - 1) * 10;
            const { data } = await getvillagevisitAPI({ offset, limit: 10 });
    ...
    const villagvisitssearch = async () => {
        try {
            document.body.classList.add("loading-indicator");
            ...
            if (fromDate && toDate) {
                ...
                const { data } = await getvillagevisitAPI();
                if (data.status === true) {
    ...
    const downloadExcel = async () => {
        try {
            ...
            const dataToDownload = tickets.map(ticket => ({
                ...
                'Purpose of Visit': ticket.purposeVisit || '-',
                'G/P Program': ticket.gpProgram || '-',
                'program Description': ticket.proDesc || '-',
                'program Inchagre': ticket.proInchagre || '-',
                'program Incharge Phone': ticket.proInchagrePhone || '-'
            }));
    
            // Convert data to XLSX format
            const worksheet = XLSX.utils.json_to_sheet(dataToDownload);
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'Village Visiting Reports');
            ...
            saveAs(blob, 'villagevisit_reports.xlsx');
                    ...
                    <div id="content">
                        ...
                        <div className="container-fluid">
                            <div className="d-flex align-items-center justify-content-between mb-4">
                                <h1 className="h3 mb-2 text-gray-800">MLA Village Visit Reports</h1>
                                ...
                                <form id="myForm" onSubmit={e => e.preventDefault()}>
                                        <div className="row justify-content-center">
                                            ...
                                            <div className="col-md-2">
                                                <div className="form-group">
                                                    <div className="inner-addon ">
                                                        <input
                                                            type="text"
                                                            id="villageSearchTerm"
                                                            name="villageSearchTerm"
                                            ...
                                            <div className="col-md-1">
                                                <button type="submit" className="btn btn-primary mb-3" onClick={villagvisitssearch}>Search</button>
                                            </div>
                                        </div>
                                    </form>
                                        ...
                                        <table id="exportTable dataTable" className="table table-bordered table-hover" width="100%" cellspacing="0">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>program Inchagre</th>
                                                    <th>ProIncharge Phone</th>
                                                </tr>
                                            </thead>
                                                <tbody>
                                                    {renderTickets()}
...
export default VillageVisitReportsPage;